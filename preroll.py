...
def grabthename(current):
	titleplace = current.find("'title':")
	titleplaceend = current.find("', 'state': 5", titleplace + 10)
	name = current[(titleplace +10):titleplaceend]
	print(name)
	movieadd(name)
...
#building the preroll video emptying the root folder of all downloaded contents and checking the preroll folder has the most recent 25 pre rolls
	def buildpreroll(stream, filelocation):
		...
		preroll = ffmpeg.drawtext(preroll, text= title, fontfile=titlefont, x=(1106 - titleoffset), y=20, escape_text=True, fontcolor="0xFFFFFF@0xff", fontsize= 76, enable='gte(t,1)')
		preroll = ffmpeg.drawtext(preroll, text= description, fontfile=descriptionfont, x=3, y=500, escape_text=True, fontcolor="0xFFFFFF@0xff", fontsize= 22, enable='gte(t,1)')
		preroll = ffmpeg.overlay(preroll, fadeout)
		preroll = ffmpeg.output(prerollaudio, preroll, ("{}prerolls/{} Preroll.mp4".format(folder, title)))
		ffmpeg.run(preroll)
		...
		settinginput = ("{}; {}prerolls/{} Preroll.mp4".format(plexsetting, folder, title))
...
#looking up the top movie theatricle trailer with some extra stipulations to avoid playlist videos etc
	query_string = urllib.parse.urlencode({"search_query" : name + " Theatrical Trailer {}".format(year)})
	html_content = urllib.request.urlopen("http://www.youtube.com/results?" + query_string + "&sp=EgIgAQ%253D%253D")
...
#looking up the top soundtrack youtube video
	...
	music_html_content = urllib.request.urlopen("http://www.youtube.com/results?" + music_query_string + "&sp=EgIQAQ%253D%253D")
	music_search_results = re.findall(r'href=\"\/watch\?v=(.{11})', music_html_content.read().decode())
...
#downloading the soundtrack for the movie from the top result found in the search
	...
	music_yt.streams.get_highest_resolution().download(output_path=("{}".format(folder)),filename="{} sounndtrack".format(name))
...
#downloading the movie theatricle trailer from the top result foind in the search
	...
	yt.streams.get_highest_resolution().download(output_path=("{}".format(folder)),filename="{}".format(name))
